...
export default function PastReviews() {
  ...
  const formatReview = (r: any): Review => ({
    ...
    wallet: r.casino_wallet || r.wallet || 'BP77',
            ...
            >
              {reviews.map((review, index) => (
                <div key={review.id || index} className="relative min-w-[20%] max-w-[20%] flex-shrink-0">
                  {index === 0 && (
                    <span className="absolute -top-4 right-4 z-30 bg-gradient-to-r from-[#6EC9FF] via-[#4FA8FF] to-[#1E4F8F] text-white text-xs font-semibold px-4 py-1.5 rounded-full shadow-md animate-pulse">
                      Latest Review
                    </span>
                  )}
          ...
          >
            {reviews.slice(0, expandedIndex === -1 ? 10 : 5).map((review, index) => (
              <div key={review.id || index} id={`review-${index + 1}`} className="relative transition-all duration-500">
                {index === 0 && (
                  <span className="absolute -top-3 right-4 z-30 bg-gradient-to-r from-[#6EC9FF] via-[#4FA8FF] to-[#1E4F8F] text-white text-[11px] font-semibold px-3 py-1 rounded-full shadow-md animate-pulse">
                    Latest Review
                  </span>
                )}
    ...
    >
      ...
      <div className="flex items-center justify-between border-t border-white/20 pt-3">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <img src="/australia-flag.png" alt="Flag" className="w-5 h-5 sm:w-6 sm:h-6" />
          <span
